...
<%@ include file="include/review_ajex.jsp"%>
...
<script type="text/javascript">
	...
	function likeclick(number){
		...
		var thumb_up = document.querySelector('#thumb_up'+number);
		var thumb_down = document.querySelector('#thumb_down'+number);
		if(thumb_up.className === 'thumbs'){
			if(thumb_down.className === 'thumbs'){
				thumb_up.className= 'btn-btn-secondary';
			}else{
				// 좋아요 버튼에 불이 안들어와 있는데 싫어요 버튼에 불이 들어와 있으면
				thumb_down.className = 'thumbs'
				thumb_up.className='btn-btn-secondary';
			}
		}else{
			//이미 좋아요 버튼에 불이 들어와 있으면
			thumb_up.className='thumbs'
		}
		
	}
	function unlikeclick(number){
		var thumb_up = document.querySelector('#thumb_up'+number);
		var thumb_down = document.querySelector('#thumb_down'+number);
		if(thumb_down.className === 'thumbs')	{
			if(thumb_up.className === 'thumbs'){
				thumb_down.className= 'btn-btn-secondary';
			}else{
				//좋아요 버튼에 불이 들어와 있으면
				thumb_up.className='thumbs'
				thumb_down.className='btn-btn-secondary';
		...
		<div id="reviews" class="review-section">
			<div class="d-flex align-items-center justify-content-between mb-4">
				...
				<select
					...
					<option data-select2-id="3">최신순</option>
					<option>별점순</option>
				</select>
			</div>
			<div class="row">
					...
					<li>
						<div class="d-flex">
							...
							likeratio();
								...
								<button id="thumb_down${status.index}" class="thumbs" onclick="check_member_Unlikereview(${status.index})" value="-1">
